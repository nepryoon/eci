...
def test_scenario1_who_calls_validate_finds_process(retrieval_engine_addr, vllm_fake_url):
    result = run_ask("Chi chiama Validate?", retrieval_engine_addr, vllm_fake_url)
...
def test_scenario2_end_to_end_determinism(retrieval_engine_addr, vllm_fake_url):
    result1 = run_ask("Chi chiama Validate?", retrieval_engine_addr, vllm_fake_url)
    result2 = run_ask("Chi chiama Validate?", retrieval_engine_addr, vllm_fake_url)
...
def test_scenario3_no_results_no_llm_call(retrieval_engine_addr, vllm_fake_url):
    ...
    result = run_ask("zzzznonexistentqueryterm12345", retrieval_engine_addr, vllm_fake_url)
...
def test_scenario4_retrieval_engine_unreachable(vllm_fake_url):
    with pytest.raises(RetrievalUnavailableError) as excinfo:
        run_ask("query qualunque", UNREACHABLE_GRPC_ADDR, vllm_fake_url)
...
def test_scenario5_llm_unreachable_sources_still_available(retrieval_engine_addr):
    with pytest.raises(LLMUnavailableError) as excinfo:
        run_ask("Chi chiama Validate?", retrieval_engine_addr, UNREACHABLE_HTTP_URL)
...
def test_forward_expand_from_validate_finds_no_callers(retrieval_engine_addr):
    ...
    definizione)."""
    security_context = build_security_context()
    channel = grpc.insecure_channel(retrieval_engine_addr)
    try:
...
def test_eci_command_full_ask_via_subprocess(retrieval_engine_addr, vllm_fake_url):
    """`eci ask` end-to-end come processo REALE (non `run_ask` in-process):
    stesso env di config del resto della suite, output su stdout."""
    eci_bin = Path(sys.executable).parent / "eci"
    env = {
        **os.environ,
        "ORCHESTRATOR_RETRIEVAL_ADDR": retrieval_engine_addr,
        "ORCHESTRATOR_VLLM_URL": vllm_fake_url,
    }
    ...
    assert "[vllm-fake risposta deterministica]" in result.stdout
    assert "Fonti:" in result.stdout
