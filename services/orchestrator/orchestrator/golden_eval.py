...
def _facts(entry: GoldenEntry) -> tuple[str, ...]:
    return tuple(value for values in entry.expected_facts.values() for value in values)
...
def run_golden_eval(
    ...
    *,
    ...
    summary: dict[str, float | int | bool] = {
        ...
        "latency_p95_ms": latencies[max(0, int(len(latencies) * 0.95) - 1)] if latencies else 0.0,
    }
