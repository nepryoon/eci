...
| `task test` | FAIL | tenta testcontainers Keycloak, OPA e orchestrator/Neo4j; daemon Docker assente; le restanti suite osservate passano |
...
|---|---|---|---|---|---|---|---|---|---|
| Superficie build/lint/test completa e ADD reale protetto | ADD registro D1–D9; PLAYBOOK T0.1; harness §6.9 | scripts, Taskfile, CI, tutti i manifest | aggregati correnti verdi solo sui moduli enumerati | tool `gc-postgres`/`reconcile`, fakes, `eci-common`, E2E/interop e vari integration omessi; unit target dipende da Docker; guard usa path inesistente | inventario manifest, guard sintetico, target CPU e integration | suite security non può essere accidentalmente omessa | prima riparazione | in_progress | SPEC-068; baseline sopra |
...
| Semantica completa `ImpactAnalysisRequest` | D7; contratto protobuf; ADD M2 §§1.2–1.6 | retrieval-engine, generated clients | BFS bounded con `max_depth`/`max_nodes`, auth scope e singolo repo | edge_types, direction, fanout per-hop, intersezione multi-repo, min score, source hydration, path completo, ImpactKind, truncation e status errori | unit + Neo4j integration + auth/cancel adversarial | filtri pre-query e nessuna hydration cross-scope | SPEC-068, poi prima dei runtime query | open | audit mostra commenti espliciti "solo primo repo" e campi ignorati |
| Tombstone/delete end-to-end | ADD M1 §§1.6.4–1.6.6, 2.2; event schema `DELETE`; ADR-0023 limite | ingestion, PG/outbox, 3 sink, cache, summary, reconcile | schema accetta DELETE; reconcile rimuove alcuni extra projection rows | file rimosso non produce mutazione/outbox; sink consumer DELETE assente; invalidazioni incomplete | ACID/replay/idempotenza/failure/cross-tenant integration | delete namespaced; marker dopo effetto | Impact audit e runtime ingestion | open | SPEC dedicati per canonical delete e ciascun sink/invalidation |
