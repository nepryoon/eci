...
| Tombstone/delete end-to-end | ADD M1 §§1.6.4–1.6.6, 2.2; event schema `DELETE`; ADR-0025; SPEC-070–076 | ingestion, PG/outbox/CDC, shared Go metadata, embedding worker, 3 sink, cache, summary, reconcile | canonical DELETE/CDC/parser; graph scoped deletion; embedding delete acknowledgement; Qdrant scoped Delete(wait=true); OpenSearch synchronous delete-by-query with ID+tenant/repo/ACL, refresh, partial-result rejection and marker post-effect | esecuzione Docker-backed bloccata; cache/summary invalidation e reconciliation delete semantics restano | ACID/replay/idempotenza/failure/cross-tenant integration, CDC header reale, invalidation/reconcile | metadata non-disclosing; server-side graph/vector/search scope gates; delete non ricrea dati; marker post-effetto | SPEC-070/071/073–076 implemented, SPEC-072 verified; invalidation/reconcile SPEC successive | in_progress | search red/integration/feature `07e90b7`/`861c966`/`66a7f83`; unit/race/vet, integration compile, build/lint/test/security PASS; Docker blocked |
| T7.1b orchestrator runtime | PLAYBOOK T7.1b; ADD M2/M3/M4; SPEC-077 | orchestrator, libs/py, chart | grafo T5.1 e CLI verificati; boundary Python metadata-only + OPA fail-closed verificato con 18 test e incluso nel gate security | contratto/API long-running streaming, probe dipendenze, limiti, shutdown, OTel, workload | unit/integration/security/render | contesto solo metadata autenticati, duplicate/malformed fail before OPA, PDP failure fail-closed | SPEC-077 verified + Impact semantics | open | SPEC-077 `99974bb`/`3bfe0ee`/`dcee4c0`; aggregate verdi; nuovo SPEC runtime |
