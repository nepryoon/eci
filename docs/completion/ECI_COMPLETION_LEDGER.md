...
| Tombstone/delete end-to-end | ADD M1 §§1.6.4–1.6.6, 2.2; event schema `DELETE`; ADR-0025; SPEC-070–073 | ingestion, PG/outbox/CDC, shared Go metadata, 3 sink, cache, summary, reconcile | canonical DELETE + CDC operation + parser fail-closed implementati; sink-graph applica node scope-check/DETACH DELETE e relation whitelist DELETE, lock senza placeholder, GDS generation condizionale e marker post-effetto | esecuzione PostgreSQL/CDC/Neo4j bloccata dal daemon; embedding/Qdrant/OpenSearch DELETE e invalidazioni/reconciliation restano | ACID/replay/idempotenza/failure/cross-tenant integration, CDC header reale, projection consumers | tombstone senza source/vector; metadata non-disclosing; node scope completo; Cypher type whitelist; marker dopo effetto | SPEC-070/071/073 implemented, SPEC-072 verified; projection SPEC successive | in_progress | graph red/feature `19406cf`/`e1d71a5`; unit/race/vet, integration compile, build/lint/test/security PASS; Docker aggregate bloccato |
