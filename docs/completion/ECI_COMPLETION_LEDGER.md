...
| Tombstone/delete end-to-end | ADD M1 §§1.6.4–1.6.6, 2.2; event schema `DELETE`; ADR-0025; SPEC-070 | ingestion, PG/outbox, 3 sink, cache, summary, reconcile | canonical DELETE autenticata implementata: receipt e node/relation/chunk/embedding tombstone ACID, FK cleanup, replay/conflict, no MinIO/parser; envelope condiviso allineato ai quattro aggregate; CPU gates verdi | esecuzione PostgreSQL bloccata dal daemon; CDC deve promuovere event type; sink DELETE e invalidazioni/reconciliation restano | ACID/replay/idempotenza/failure/cross-tenant integration, poi consumer effect-before-marker | scope solo da header; delete selezionata per tenant/repo/ACL/path; tombstone senza source/vector; marker dopo effetto | SPEC-070 implementata; projection SPEC successive | in_progress | red tests commit `70cb799`; feature `4ad4c08`; `task build/lint/test/guard`, schema/codegen e compile integration PASS; `task test:integration` fallisce esplicitamente sul daemon assente |
