...
2. **Dato** lo stesso identico messaggio riconsegnato (redelivery at-least-once, es. riavvio del consumer prima del commit dell'offset), **quando** viene riprocessato, **allora** `processed_events` blocca la seconda scrittura (nessuna riga Neo4j duplicata, nessun errore) — verificabile contando i nodi con quell'`id` prima e dopo.
