...
**Indice**: nome dichiarato `code_chunks`, creato idempotentemente al via del servizio (stesso principio già stabilito per la collection Qdrant in T3.1) se non esiste. Mapping minimo: `text` (analizzato per full-text), `entity_id`/`provenance`/`chunk_index` (memorizzati, non necessariamente analizzati per full-text), `event_sequence` long e `chunk_id` keyword. La sequenza canonica permette di scegliere la proiezione corrente durante la convergenza di una sostituzione; `chunk_id` duplica il DocumentID canonico in un campo con doc values, necessario come tie-break finale deterministico PIT/`search_after` di SPEC-069 perché OpenSearch non permette di ordinare sul metadata `_id`.
...
1. **Dato** un messaggio `CodeChunk` reale, **quando** il servizio lo consuma, **allora** un documento esiste in OpenSearch con quell'id, `chunk_id` uguale all'id, `event_sequence` uguale al metadata canonico, `text` corrispondente, `entity_id`/`provenance` nei campi memorizzati.
