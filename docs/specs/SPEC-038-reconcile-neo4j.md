...
3. **`Republish`: la provenance canonica viene copiata integralmente dalla riga `code_node`.** `code_node` non ha una colonna `file_path` scalare: percorso e scope autenticato vivono dentro `provenance` JSONB. La query legge `domain, node_type, name, ast_hash, provenance` e incorpora la `provenance` come JSON invariato. Una riparazione non può quindi perdere `tenant_id`, `repo` o `acl_group`; i sink fail-closed ricevono la stessa struttura prodotta da `persist_parsed_file`.
