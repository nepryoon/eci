...
- embedding-worker mTLS identity consumed its retry topic and accessed its exact
  ...
  requires bounded native TEI `/health` before its consume-loop, after
  uncommitted failures and in readiness; TEI unavailability stays uncommitted, resets the reader before
  later offsets, and bypasses retry/DLQ: PASS (deterministic unit evidence; first-party image not deployed);
